...
@app.on_message(command("رفع كس"))
async def rf3nmla(client:Client, message:Message):
  
  if message.reply_to_message.from_user.mention in kos:
    kos.append(message.reply_to_message.from_user.mention)
  await message.reply_text(f"**تم رفع العضو\n│ \n└ʙʏ : {message.reply_to_message.from_user.mention}\n\n كس من قبل {message.from_user.mention}😂😂**")


@app.on_message(command("ت كس"))
async def tnzelnmla(client:Client, message:Message):
  if message.reply_to_message.from_user.mention in kos:
    kos.remove(message.reply_to_message.from_user.mention)
  await message.reply_text(f"**تم تنزيل العضو\n│ \n└ʙʏ : {message.reply_to_message.from_user.mention}\n\n من قائمة الاكساس 😂😂  \n\n لعرض القائمه اكتب قائمة الاكساس**")


@app.on_message(command("قائمة الاكساس"))
async def nml(client:Client, message:Message):
  ko = ""
  for n in kos:
      ko += n + "\n"
  await message.reply_text(f"**قائمة الاكساس في البار 😂😂 \n\n{ko}**")
